...
udx__cirbuf_init (udx_cirbuf_t *c, uint32_t initial_size) {
    ...
    c->values = calloc(initial_size, sizeof(udx_packet_t *));
}
...
udx__cirbuf_set (udx_cirbuf_t *c, udx_packet_t *val) {
    udx_packet_t **values = c->values + (val->seq & c->mask);
    udx_packet_t *v = *values;
    ...
    udx_packet_t **old_values = c->values;
    ...
    c->values = calloc(c->size, sizeof(udx_packet_t *));
    c->values[val->seq & c->mask] = val;
    for (uint32_t i = 0; i < old_size; i++) {
        udx_packet_t *v = old_values[i];
